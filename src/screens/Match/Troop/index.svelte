...
{#if troop.HP > 0 || troop.Player == ""}
  <div class="absolute z-[100] w-[20px] h-[20px] border-[1px] {troop.Player == player_id ? 'bg-blue-500 border-blue-700' : 'bg-red-500 border-red-700'}" 
    style:top={`${startY + $position.y}px`} style:left={`${startX + $position.x}px`}
    style={`transform: rotate(${angle}rad);`}
  >
  </div>

  <div class="{troop.Player === '' ? 'hidden' : ''} text-center absolute z-[90] w-[60px] h-[20px] border-[1px] {troop.Player == player_id ? 'bg-blue-300 border-blue-500' : 'bg-red-300 border-red-500'}" 
